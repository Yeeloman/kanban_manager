...
<Dialog.Root>
  <Dialog.Trigger>
    <Button size="rounded" variant="minimalAdd" class="w-full h-full">
      <div class="px-4 font-bold text-3xl text-gray-400 dark:hover:text-purp_manager-def hover:text-purp_manager-def">
        <Edit/>
      </div>
    </Button>
  </Dialog.Trigger>
  <Dialog.Content class="{$bgDialogCss}">
    ...
    <div class="space-y-5">
      ...
      <div class="space-y-2">
        ...
        <Textarea placeholder="e.g. Be like water my friend" class="w-full focus:border-purp_manager-def border-gray-500" />
      </div>
      <div class="space-y-2">
